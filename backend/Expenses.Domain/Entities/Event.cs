...
namespace Expenses.Domain.Entities
{
    ...
    public class Event
    {
        ...
        public string CreatorId { get; }    // use string instead of value object or we need manual editing of migration
        ...
        public Event(string creatorId, string title, string description, DateTime startDate, DateTime endDate, string currency)
        {
            if (string.IsNullOrEmpty(creatorId)) throw new EventValidationException("CreatorIdInvalid", Localization.Language.InvalidCreator);
            ...
            _participants = new List<string> { creatorId };
            Users = new List<User> { new User(creatorId) };
        ...
        {
            var p = _participants.Find(p => p == participant.Id);
            if (p != null)
        ...
        public void AddExpense(Expense expense)
        {
            ...
            foreach (var user in users)
                if (_participants.Find(p => p == user.Id) == null) throw new EventValidationException("AddExpenseUnknownParticipant", "User does not participate in event.");
